...
ethresh = 5e-5  # threshold for the energy std and drift
...
# reading reference from Tomas Jira and his ACRON code
...
pop_ad_int = [interp(x=t, xp=pop_ad[0], fp=pop_ad[1 + state]) for state in range(3)]
pop_ref_int = [interp(x=t, xp=ref_pop[0], fp=ref_pop[1 + state]) for state in range(3)]

# calculating standard deviation
pop_std = max([std(pop_ad_int[state] - pop_ref_int[state]) for state in range(3)])
