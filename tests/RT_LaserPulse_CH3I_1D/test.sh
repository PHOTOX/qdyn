...
test_result=$(python3 check_ch3i.py)

if [[ $test_result == "True" ]]
then
  echo "Test passed - adiabatic populations, <x> and <delta x> match the reference." > results.dat
elif [[ $test_result == "False" ]]
then
  echo "Test failed - adiabatic populations, <x> and <delta x> don't match the reference." > results.dat
else
  echo "Test failed - error in python script, it prints '$test_result' instead of True or False." > results.dat
fi

rm ener* wf* field.dat pop*
