...
test_result=$(python3 check_autocorr.py)

if [[ $test_result == "True" ]]
then
  echo "Test passed - autocorrelation function and spectrum match the reference." > results.dat
elif [[ $test_result == "False" ]]
then
  echo "Test failed - autocorrelation function and spectrum don't match the reference." > results.dat
else
  echo "Test failed - error in python script, it prints '$test_result' instead of True or False." > results.dat
fi

rm energies.dat pop_ad.dat pop_diab.dat autocorr_function.dat
